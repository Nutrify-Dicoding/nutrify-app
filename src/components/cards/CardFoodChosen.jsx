...
function CardFoodChosen() {
	...
	const fat = (selectedFood.fat * porsi).toFixed(2).toString();
	const calorie = (selectedFood.cal * porsi).toFixed(2).toString();
	const protein = (selectedFood.protein * porsi).toFixed(2).toString();
	const carbo = (selectedFood.carb * porsi).toFixed(2).toString();

	return (
		<div
			className={`flex justify-between items-center tab:flex-col w-full mt-5 border border-white-400/60 rounded-[10px] p-4 sm:p-3 ${
