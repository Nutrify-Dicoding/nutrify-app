...
const CardFoodChosen = () => {
	...
	return (
		<>
			<div className={`flex justify-between items-center tab:flex-col w-full mt-5 border border-white-400/60 rounded-[10px] p-4 sm:p-2 ${selectedFood.food_id ? '' : 'hidden'}`} >
				<div className="w-[25%] 2lg:w-[27%] tab:flex tab:justify-between tab:items-center tab:w-full">
					<div className="flex flex-row gap-5 items-center">
						<img src={selectedFood.image && selectedFood.image.includes('http') ? selectedFood.image : 'https://picsum.photos/150/150'} alt={'Gambar Makanan'} className="w-[70px] 2lg:w-14 h-[70px] 2lg:h-14 rounded-full" />
					...
					<div className="hidden py-2 px-2 rounded-full bg-slate-200 tab:flex flex-row items-center justify-between">
						<button className="w-11 sm:w-8 font-bold text-orange-100" onClick={() => hanldeMinPorsi()}>
							-
						</button>
						<p>{porsi}</p>
						<button className="w-11 sm:w-8 font-bold text-orange-100" onClick={() => hanldePlusPorsi()}>
							+
						</button>
					</div>
				</div>
				...
				<div>
					...
					<div>
						<button onClick={() => Track()} className='p-3 py-2 mt-1 text-white w-full bg-orange rounded-full'>Submit</button>
					</div>
				</div>
			</div>
		</>
