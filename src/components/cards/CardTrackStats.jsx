...
const CardTrackStats = ({
  ...
  return (
    <div className="w-64 md:w-40 md:h-28 h-52 border border-gray-200 rounded p-7 justify-between flex-col">
      <div className="w-full flex flex-row justify-between items-center h-14 md:h-10 border-b border-gray-200">
        <div className="flex flex-row items-center gap-3">
          <img src={`icons/${icon}`} alt="" />
          <p className="text-gray-500 md:hidden block font-semibold">
            {name}
          </p>
        </div>
      ...
      <div className="flex flex-row justify-between items-center h-3/4">
        <p className={`font-bold text-4xl md:text-xl ${color}`}>
          {value}
        </p>
        <p className="text-xs text-green">/20000</p>
        <p className="w-20 md:hidden block text-sm  text-gray-400">
