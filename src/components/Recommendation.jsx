    ...
    return (
        <div className="mt-12 mb-20">
            ...
            <div className="grid mt-8 sm:grid-cols-1 tab:grid-cols-3 grid-cols-4 gap-5 mb-12">
                {foods.slice(0, 4).map((item) => <CardFood key={item._id} id={item._id} name={item.name} description={item.desc} fat={item.fat} calorie={item.cal} protein={item.protein} carbohydrate={item.carb} image={item.image.includes('http') ? item.image : 'https://picsum.photos/265/150'} />)}
