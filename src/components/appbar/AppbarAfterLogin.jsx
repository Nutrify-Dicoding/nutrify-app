			...
			<nav className="bg-white border-gray-200 border-b-[1px] w-full px-[6.25%] fixed top-0 z-10">
				<div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto py-4">
					...
					<div className="flex flex-row gap-5 items-center ">
						<Link to={"/"} className="flex sm:hidden cursor-pointer text-base">Home</Link>
						<Link to={"/track"} className="flex sm:hidden cursor-pointer text-base">Tracking</Link>
						<button
							type="button"
						...
						<div>
							<img src="/icons/drop-icon.svg" className={`cursor-pointer transition ${isOpen?'rotate-180':''}`} onClick={() => { setIsOpen(!isOpen) }} />
							{/* sementara */}
							<div className={`absolute ${isOpen ? 'block' : 'hidden'}`}>
								<ul className="mt-4 ms-[-50px] lg:ms-[-80px] bg-white">
									<li className="py-2 px-5 border"><Link to={"/profile"}>Profile</Link></li>
									<li className="py-2 px-5 border" role="button" onClick={()=>logout()}>Logout</li>
								</ul>
							</div>
						</div>
					</div>
