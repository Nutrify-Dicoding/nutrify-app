	...
	return (
		<nav className="bg-white border-gray-200 border-b-[1px] w-full px-[6.25%] fixed m-0 z-10">
			<div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto py-4">
				...
				<div
					className={`items-center justify-between md:w-full flex w-auto md:order-3 overflow-hidden md:max-h-0 ${
						navbarOpen ? 'md:max-h-[300px]' : ''
					}`}
					id="navbar-cta"
				>
					<ul
						className={`flex md:flex-col space-x-8 md:space-x-0 font-medium md:p-4 p-0 md:mt-4 rounded-lg md:bg-gray-50 rtl:space-x-reverse flex-row mt-0 md:border-0 bg-white md:w-full transition-all ease-out duration-700 md:overflow-hidden md:max-h-0 ${
							navbarOpen ? 'md:max-h-[300px]' : ''
						}`}
					>
						<li>
