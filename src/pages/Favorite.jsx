...
function Favorite() {
    ...
    return (
      <section className="pt-24 px-[6.25%] text-navy mb-20 sm:mb-12">
          ...
          <div className="w-full grid grid-cols-4 gap-5 mt-3 tab:grid-cols-3 md:grid-cols-2 sm:grid-cols-1" id="list-food">
              {favorite.map((item, index) => <CardFood key={index} id={item.food._id} name={item.food.name} description={item.food.desc} fat={item.food.fat} calorie={item.food.cal} protein={item.food.protein} carbohydrate={item.food.carb} image={item.food.image.includes('http') ? item.food.image : 'https://picsum.photos/265/150'} />)}
