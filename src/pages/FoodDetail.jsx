...
function FoodDetail() {
	...
	const toogleFavorite = () => {
		...
		const request = favorited ? axios.delete(`/favorite/${favoriteID}`, config) : axios.post('/favorite', formData, config);
		request.then((res) => {
			...
			<div className="2xl:mx-auto">
				<div className="w-full flex tab:flex-col">
					...
					<div className="w-[50%] lg:w-[55%] tab:w-full pr-[6.25%] tab:px-[6.25%] pl-16 sm:px-[6.25%] md:py-5">
						...
						<h1 className="text-3xl sm:text-mobile-4xl font-semibold text-navy mb-1">{foodDetail.name ?? ''}</h1>
						<div>
							<p className="text-navy">{foodDetail.desc ?? ''}</p>
							<h2 className="text-xl sm:text-mobile-xl mb-4 mt-4 text-navy font-semibold">Nutrisi Utama</h2>
							<div className="grid  grid-cols-4 lg:grid-cols-2 tab:grid-cols-4 sm:grid-cols-2 gap-8 bg-slate-100 p-4 text-center rounded-lg border-b-[1.5px] border-white-100 ">
								<div>
									...
									<div className="text-xs text-white-500">{percentNutrition.fat ?? 0}% Harian</div>
								</div>
								<div>
									<div className="text-navy">
									...
									<div className="text-xs text-white-500">{percentNutrition.cal ?? 0}% Harian</div>
								</div>
								<div>
									<div className="text-navy">
									...
									<div className="text-xs text-white-500">{percentNutrition.protein ?? 0}% Harian</div>
								</div>
								<div>
									<div className="text-navy">
									...
									<div className="text-xs text-white-500">{percentNutrition.carb ?? 0}% Harian</div>
								</div>
							</div>
							...
							<div className="pt-11 flex items-center tab:justify-end">
								...
								<button
									type="button"
									className="me-2 ml-4 py-2 px-3 border-orange border-2 rounded-lg"
