    ...
    return (
        <section className="px-[6.25%] w-full tab:h-full tab:pt-[4rem] mt-[7.5rem] tab:mt-14 ">
            <p className="text-2xl font-bold text-navy">
                Pilih Makanan
                <Link to="/alltrack">
                    <button type="button" className="p-1 px-2 text-white text-md font-medium bg-orange rounded-md ms-4">All Tracking</button>
                </Link>
            </p>
            <CardFoodChosen />
            ...
            <div>
                ...
                {/* <CardFoodHistory /> */}
                {foodsSelectedToday.map((x, index) => <CardFoodHistory key={index} data={x.foodId} portion={x.portion} />)}
            </div>
            <Recommendation />
        </section>
    );
